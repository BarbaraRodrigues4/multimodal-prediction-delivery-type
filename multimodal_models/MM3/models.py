...
class TimmModel(nn.Module):
    def __init__(self, num_classes=2, max_img_num_per_col=3, use_learnable_image=True):
        ...
        self.feature_dim = self.base_model.num_features  # 768 for vit_base_patch16_clip_224
    ...
    def forward(self, x, image_valid_num=None, return_feature=False):
        B, N, C, H, W = x.shape  # N should be 2
        device = x.device
...
class FusionMLP(nn.Module):
    def __init__(
        ...
        super().__init__()
        # Feature adaptation (project both to max-dim and concatenate)
